...
bool check_equal(smt::SmtSolver &slv,
                 const smt::Sort &astSort,
                 const Theory &t,
                 const smt::Term &smt_term,
                 const Expr &expr)
{
    smt::Term term = construct(slv, astSort, t, expr);
    slv->assert_formula(slv->make_term(smt::Equal, smt_term, term));
    return slv->check_sat().is_sat();
}
    ...
    const int &depth)
{
    ...
    for (int i = 1; i != std::max(2, static_cast<int>(t.rules.size() + 1)); i++)
    {
        for (auto &&d : fr)
        {
