...
/**
 * For testing: confirm that a given expression yields a particular SMT-switch term
 * when being built by construct()
 *
 * @param slv
 * @param astSort generated by create_datatypes
 * @param t - `Theory` which `expr` comes from
 * @param smt_term - A SMT-lib term, should be non-error/none AST type
 * @param expr - A C++ Expr
 * @returns whether or not the constructed expr is provably equal to the smt_term
 */
